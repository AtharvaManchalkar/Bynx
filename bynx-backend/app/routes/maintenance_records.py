...
async def add_maintenance_record(record: MaintenanceRecordCreate, role: str = Depends(get_user_role)):
    if role != 'Worker':
        raise HTTPException(status_code=403, detail="Not authorized")
    return create_maintenance_record(record)

@router.put("/maintenance/{record_id}", response_model=MaintenanceRecordResponse)
async def update_record(record_id: int, record: MaintenanceRecordUpdate, role: str = Depends(get_user_role)):
    if role != 'Admin':
        raise HTTPException(status_code=403, detail="Not authorized")
    updated_record = update_maintenance_record(record_id, record)
